  ...
  describe("Edit asset Description Tests", function () {
    ...
    it("should edit description successfully", async function () {
      ...
      await driver.actions()
        .move({origin: descriptionSection})
        .pause(500)
        .perform();
      ...
      const textarea = await driver.wait(
        until.elementLocated(By.id('description-input')),
        5000
      );
      // Clear using JavaScript executor
      ...
      await sleep(1000);
      // 5. Verify update
      const updatedText = await driver.wait(
        until.elementLocated(By.id('asset-description-container')),
        5000
      ).getText();
      await sleep(1000);
      assert.strictEqual(
        updatedText,
        testText,
        `Description should be updated to "${testText}"`
      );
    });
  });
});
