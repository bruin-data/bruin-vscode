  ...
  >
    ...
    <div v-if="isExpanded" class="mt-4 overflow-y-auto" style="max-height: 200px">
      <div v-for="(warning, index) in formattedWarningMessages" :key="index">
        ...
        <div class="ml-5 mt-2">
          <div
            v-for="(issue, issueIndex) in Object.values(warning.issues).flat()"
            :key="issueIndex"
            class="mb-2"
          >
...
console.log("props.warnings", props.warnings);
const isExpanded = ref(false);

const formattedWarningMessages = ref<FormattedErrorMessage[]>([]);

const parsedWarnings = computed(() => {
  if (!props.warnings) return [];
  try {
    const warningObject = props.warnings;
    return warningObject.map((warning) => {
      return {
        ...warning,
        expanded: true,
...
formattedWarningMessages.value = parsedWarnings.value;
...
const toggleIssueExpansion = (warningIndex: number, issueIndex: number) => {
  formattedWarningMessages.value[warningIndex].issues[issueIndex].expanded =
    !formattedWarningMessages.value[warningIndex].issues[issueIndex].expanded;
};
</script>
