<template>
  <div
    :class="`flex items-center gap-x-1.5 rounded-md px-2.5 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 
  ...
  >
    ...
    >
      <div v-show="status" class="flex-grow">
        ...
        <span v-if="status === 'loading'" class="flex-shrink-0 spinner"></span>
      </div>
      <div class="flex-grow">
        {{ buttonLabel }}
      </div>
